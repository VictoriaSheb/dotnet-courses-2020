...
namespace Task1
{
    public partial class MainForm : Form
    {
        ...
        { 
            ...
            if (propertyName!="")
                Sort(typeof(User).GetProperty(propertyName), sortModeUser, GetDataSourse<User>(dgvUser), dgvUser);
        }
        ...
        private void dgvReward_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            Sort(typeof(Reward).GetProperty(propertyName), sortModeReward, GetDataSourse<Reward>(dgvReward), dgvReward);
        }
        ...
        private void dgvUser_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            {
                User user = (User)dgvUser.SelectedCells[0].OwningRow.DataBoundItem;
                //  combo.DataSource = GetDataSourse<User>(dgvUser).ToList()[idRowForComboBox].RewardsUser;
                combo.DataSource = usersBLL.GetRewardsFromUser(user.Id);
        ...
        private BindingList<T> GetDataSourse<T>(DataGridView view) 
        {
            return(BindingList<T>)view.DataSource;
        }
